trigger OppSyncTrigger on Opportunity(after update) {
	...
	if (!startSyncQuoteMap.isEmpty()) {
		...
		String qliQuery =
			'select Id, QuoteId, PricebookEntryId, UnitPrice, Quantity, Discount, SortOrder' +
			qliFields +
			' from QuoteLineItem where QuoteId in (' +
			syncQuoteIds +
			') order by QuoteId, SortOrder ASC';
		String oliQuery =
			'select Id, OpportunityId, PricebookEntryId, UnitPrice, Quantity, Discount, SortOrder' +
			oliFields +
